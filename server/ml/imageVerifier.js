...
const FOOD_KEYWORDS = [
  ...
  'hen-of-the-woods', 'bolete', 'agaric',
...
async function verifyImage(base64Image) {
  ...
  try {
    ...
    const isFood = isFoodLabel(top.className) ? topScore >= 0.10 : foodScore >= 0.15;

    let overallVerdict, overallConfidence;
    if (isFood) {
      overallVerdict = 'Verified';
      overallConfidence = Math.max(55, Math.round(Math.max(topScore, foodScore) * 100));
    ...
    const reportStatus = overallVerdict === 'Verified' ? 'Approved' : 'Not Valid';

    const verification = {
      overallVerdict, overallConfidence,
      elaScore: 0, elaDetails: '',
      classificationLabels: predictions.map(p => p.className.split(',')[0]),
      classificationConfidence: Math.round(topScore * 100),
      isFood, matchedFoodLabel: topLabel,
      contextMatch: isFood,
      qualityScore: topScore > 0.5 ? 85 : topScore > 0.2 ? 60 : 35,
      qualityIssues: !isFood ? ['not_food'] : [],
      imageResolution: '', exifScore: 0, exifPresent: false,
      exifDetails: '', statsScore: 0, statsDetails: '',
      details: `${topLabel} (${Math.round(topScore * 100)}%)`,
      analyzedAt: new Date(),
    };

    console.log(`[ML] ✅ verdict=${overallVerdict}, confidence=${overallConfidence}%`);
    ...
    return {
      verification: {
        overallVerdict: 'Fake / Suspicious', overallConfidence: 20,
        elaScore: 0, elaDetails: '',
        classificationLabels: ['error'], classificationConfidence: 0,
        isFood: false, matchedFoodLabel: '', contextMatch: false,
        qualityScore: 0, qualityIssues: ['analysis_error'],
        imageResolution: '', exifScore: 0, exifPresent: false,
        exifDetails: '', statsScore: 0, statsDetails: '',
